...
public class SceneTreeViewer extends JPanel
    ...
    private Map<VRMLNodeType, DefaultMutableTreeNode> treeMap;
    ...
    private Appearance hiddenAlternateAppearance;
    ...
    private JComboBox<NodeFilter> filterBox;
    ...
    public SceneTreeViewer(OGLStandardBrowserCore core, NodeFilter initialFilter) {
        ...
        traverser.setObserver(SceneTreeViewer.this);

        core.addCoreListener(SceneTreeViewer.this);
        ...
        blinkButton.addActionListener(SceneTreeViewer.this);
        hideButton.addActionListener(SceneTreeViewer.this);
        showButton.addActionListener(SceneTreeViewer.this);
        centerButton.addActionListener(SceneTreeViewer.this);

        buttonPanel.add(blinkButton);
        buttonPanel.add(hideButton);
        buttonPanel.add(showButton);
        showButton.setEnabled(false); // TODO fix setShowOnlyAppearance/restoreShowAllAppearance methods
        ...
        filterBox.addActionListener(SceneTreeViewer.this);
        ...
        hiddenAlternateAppearance = new Appearance();
        Material mat = new Material();
        mat.setTransparency(0);
        hiddenAlternateAppearance.setMaterial(mat);
        ...
        // Safe to call directly as we are already in the EDT
        setVisible(true);
    ...
    public void observedNode(VRMLNodeType parent,
        ...
        Map<String, VRMLNode> defNames = curr_scene.getDEFNodes();

        // TODO: Linear search which could be bad on some scenes
        Iterator<Map.Entry<String, VRMLNode>> nodes = defNames.entrySet().iterator();
        String defName = null;

        while(nodes.hasNext()) {
            Map.Entry<String, VRMLNode> entry = nodes.next();
        ...
        if        (used && (defName != null)) {
            label = "USE " + defName;
        } else if (used && (defName == null)) { // do not display, e.g. Material under parent USE Appearance
            label = "";
...
*/
        ...
        if (!new_scene && (p_node != null)) {
            if ((label.length() > 0)) // do not show child of USE node
            {
                p_node.add(c_node);
        ...
        if (child instanceof VRMLInlineNodeType) {
            ...
            BasicScene inlineScene = inline.getContainedScene();

            sceneStack.push(inlineScene);
            VRMLWorldRootNodeType root = (VRMLWorldRootNodeType)
                    inlineScene.getRootNode();
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        if (find_parent || sgo instanceof NodeComponent) {
            ...
            while(parent != null) {
                ...
                if (userObject instanceof STNodeWrapper)
                    node = ((STNodeWrapper)parent.getUserObject()).getVRMLNode();
        ...
        if (src == blinkButton) {
            ...
            if (hiddenNode == null) {
                nodeEffects.setAlternateAppearance((Node)sgo, hiddenAlternateAppearance);
                hiddenNode = (Node) sgo;
            ...
            if (shownNode == null) {
                nodeEffects.setShowOnlyAppearance((Node)sgo, hiddenAlternateAppearance);
                shownNode = (Node) sgo;
                showButton.setText("Show All ");
                hideButton.setEnabled(false);
            } else {
                nodeEffects.restoreShowAllAppearance(shownNode);
                shownNode = null;
                showButton.setText("Show Only");
     ...
     */
    public final void runBuildTree() {
        if (running) {
    ...
    private TreePath getTreePath(TreeNode treeNode) {
        ...
        List<TreeNode> treePath = new ArrayList<>();
    ...
    public void addName(DefaultMutableTreeNode treeNode, VRMLNodeType vrmlNode) {
        ...
        if (vrmlNode instanceof BaseMetadataString) {
            ...
            String[] values = vrmlNode.getFieldValue(idx).stringArrayValues;
    ...
    public void addMetadata(DefaultMutableTreeNode treeNode, VRMLNodeType vrmlNode) {
        ...
        if (meta_node instanceof BaseMetadataString) {
            ...
            String[] value = meta_node.getFieldValue(meta_node.getFieldIndex("value")).stringArrayValues;
