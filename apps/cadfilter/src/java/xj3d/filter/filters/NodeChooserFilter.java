/*****************************************************************************
 *                        Yumetech Copyright (c) 2004 - 2009
 *                               Java Source
 *
...
/**
 * Search for the first nodes matching a node name.  Only that node
 ...
 * Limitations:
 *
 * Only works for nodes that don't contain children nodes of the same type.
 * Ie it works for Appearance but not something like Transform.
 *
 * @author Alan Hudson
...
public class NodeChooserFilter extends NewAbstractFilter {
    ...
    /** What Node are we looking for? */
    private static final String NODE_NAME = "-nodeName";

    /** What Node are we looking for? */
    private static final String DEF_OVERRIDE = "-defOverride";
    ...
    /** The name to def the node or null to not change */
    private String defOverride;

    /**
     * Basic constructor.
     */
    public NodeChooserFilter() {
     ...
     * @param defName The name to def the first node or null to not change
     */
    public NodeChooserFilter(boolean fragment, String target, String defName) {
    ...
    public void startDocument(String uri,
        ...
        if (fragmentMode) {
            ...
            // TODO: need to get a real value for this
            nodeStack.push("Appearance");
            fieldStack.push("appearance");
    ...
    public void endNode() throws SAVException, VRMLException {
        ...
        if(nodeName != null && nodeName.equals(nodeTarget)) {
            inMappedContent = false;

            // Do not issue endNode as parent will do that
        } else {
            super.endNode();
        }
    }
    ...
    public void endDocument() throws SAVException, VRMLException {
        if (fragmentMode) {
            ...
            if (!nodeStack.isEmpty())
                node = (String) nodeStack.pop();

            if (!fieldStack.isEmpty())
                field = (String) fieldStack.pop();

            if (node == null || !node.equals("Appearance")) {
                System.out.println("Node stack incorrect at end document of NodeChooserFilter  got: " + node);
            }

            if (field == null || !field.equals("appearance")) {
                System.out.println("Field stack incorrect at end document of NodeChooserFilter: ");
            ...
            switch (args[i]) {
                ...
                case DEF_OVERRIDE:
                    if (i + 1 >= args.length){
                        ...
                        throw new IllegalArgumentException(
                                "Not enough args for DEFChooserFilter.  " +
                                        "Expecting one more to specify DEF Override.");
                }   defOverride = args[i+1];
                    break;
            }
        }
    }
