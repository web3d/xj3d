 ...
 */
class CElementInt extends CElementBase implements IntContent, Appendable {
	
	/** The content int values */
	private int[] value;
	
	/** tmp storage of content */
	private ArrayList<int[]> stor;
	
	/**
	 * Constructor
	 */
	CElementInt(String tag_name, Attributes atts) {
		super(tag_name, atts);
	}
	...
	public void appendTextContent(String content) {
		...
		try {
			int[] tmp_value = FieldValueHandler.toInt(content);
			stor.add(tmp_value);
	...
	public void setTextContent(String content) {
		if (stor == null) {
			try {
				value = FieldValueHandler.toInt(content);
			} catch (NumberFormatException nfe) {
				throw new ImportFileFormatException(
					"ColladaFileParser: "+ 
					"NumberFormatException in <"+ tag_name +">: "+ 
