 ...
 */
public class ColorRGBA extends BaseEncodable implements IColor {
     ...
     */
    public ColorRGBA(String defName) {
        super("ColorRGBA", defName);
    }
        ...
        if (handler != null) {
           ...
           super.encode();
            ...
            if (color != null) {
                ...
                switch (handlerType) {
                case HANDLER_BINARY:
                    bch.fieldValue(color, num_color*4);
                    break;
                case HANDLER_STRING:
                    sch.fieldValue(FieldValueHandler.toString(color, num_color*4));
                    break;
                }
            }
            handler.endNode();
    ...
    public void setValue(String name, Object value) {
        ...
        if (name.equals("color")) {
            if (value instanceof String) {
                color = fieldReader.MFColorRGBA((String)value);
                num_color = color.length / 4;
    ...
    public Encodable clone(boolean full) {
        ...
        if (full) {
            if (color != null) {
                ...
                c.color = new float[this.num_color*4];
                System.arraycopy(this.color, 0, c.color, 0, this.num_color*4);
            }
        }
        return(c);
    }
