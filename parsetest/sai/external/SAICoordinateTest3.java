...
/**
 * A test of Coordinate, IndexedFaceSet, and set1Value on MFVec3f's.
 *
 * This version uses an IFS instead of a line set.
 *
 * Warning: The vertices for the polygons were chosen at random when the
 * geometry was changed, so it'll be a bit degenerate.
 */
public class SAICoordinateTest3 {
    ...
    // Gosh its annoying to walk down to a Coordinate node from
        ...
        X3DNode levelTwo[] = SAIUtilities.extractRootNodes(browser.createX3DFromString(
                ...
                + "          ]\n"
                + "        }\n"
                + "        coordIndex [0 1 2 -1 0 2 3 -1 0 3 4 -1 0 4 5 -1 0 5 1 -1 0 6 2]\n"
                + "      }\n"
                + "    }\n"
                + "  ]\n"
                + "}"
        ...
        coordOutput.addX3DEventListener(
                new BasicSAICoordinateMutator(browser, coordOutput, coordInput, 1)
        );
