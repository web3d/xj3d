...
public class OGLStandardBrowserCore
    ...
    public String getWorldURL() {
        ...
        // TODO: this only returns the user.dir, not the URL to the scene... fix this
        else {
            ret_val = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
                String ret_val1 = System.getProperty("user.dir");
                // Do not use URL.toExternalForm as its file:/ is
                // not right for us
                ret_val1 = ret_val1.replace('\\', '/');
                // Linux and Max already start with a leading / so
                // don't add too many
                if (ret_val1.startsWith("/")) {
                    ret_val1 = "file://" + ret_val1;
                } else {
                    ret_val1 = "file:///" + ret_val1;
                }
                return ret_val1;
...
//        sceneManager.setEnabled(true);
    ...
    public void setActiveLayers(LayerManager[] layers, int numLayers) {
        ...
        for(int i = 0; i < numLayers; i++) {
            ...
            for (SensorStatusListener sl : sensorStatusListeners) {
                l.addSensorStatusListener(sl);
            }
            for (NavigationStateListener sl : navigationStateListeners) {
                l.addNavigationStateListener(sl);
            }
            for (ProfilingListener sl : profilingListeners) {
                l.addProfilingListener(sl);
            }
            for (ViewpointStatusListener sl : viewpointStatusListeners) {
