...
/**
 *  EventInMFInt32Wrapper provides the functional implementation of
 *  EventInMFInt32 for the SimpleBrowser class.  The functionality of this class
 *  is specified in the EAI 2.0 specification.
 */


class EventInMFInt32Wrapper extends EventInMFInt32
implements ExternalEvent, EventWrapper {
    ...
    EventInMFInt32Wrapper(
        VRMLNodeType aNode, int fieldID, ExternalEventQueue aQueue, int newValue[]
    ) {
    ...
    /** Constructor for use in set1Value event queueing.  Loads the
      * current field value and then applies change to specified index.
      * @param aNode The underlying target node
      * @param fieldID The underlying target node
      * @param aQueue The queue to send events to
    ...
    private void loadValue() {
        try {
            ...
            if (storedValue==null || storedValue.length!=fieldValue.numElements)
                storedValue=new int[fieldValue.numElements];
            System.arraycopy(fieldValue.intArrayValues,0,storedValue,0,fieldValue.numElements
            );
        } catch (org.web3d.vrml.lang.InvalidFieldException ife) {
            throw new RuntimeException(
    ...
    /** @see vrml.eai.field.EventInMFInt32#setValue */
    @Override
    ...
    /** @see vrml.eai.field.EventInMFInt32#set1Value */
    @Override
    public void set1Value(int index, int newValue)
    throws ArrayIndexOutOfBoundsException {
        synchronized(theQueue.eventLock) {
            EventInMFInt32Wrapper queuedElement=
                (EventInMFInt32Wrapper) theQueue.getLast(this);
    ...
    /** Store a value in this EventIn as a place holder for the buffering
      * system.
      * @param newValue The value to store in the buffer
      */
    private void storeValue(int newValue[]) {
        ...
        System.arraycopy(newValue,0,storedValue,0,storedValue.length);
    }
