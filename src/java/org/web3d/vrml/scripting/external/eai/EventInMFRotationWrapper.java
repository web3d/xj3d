...
/**
 *  EventInMFRotationWrapper provides the functional implementation of
 *  EventInMFRotation for the SimpleBrowser class.  The functionality of this
 *  class is specified in the EAI 2.0 specification.
 */

class EventInMFRotationWrapper extends EventInMFRotation
implements ExternalEvent, EventWrapper {
    ...
    /** Two WrapperField's are equal if they point to the same actual node
      * and field */
    @Override
    ...
    private void loadValue() {
        try {
            ...
            if (
                storedValue==null ||
                storedValue.length!=(4*fieldValue.numElements)
            )
                storedValue=new float[fieldValue.numElements*4];
            System.arraycopy(fieldValue.floatArrayValues,0,storedValue,0,
                fieldValue.numElements*4
            );
        } catch (org.web3d.vrml.lang.InvalidFieldException ife) {
            throw new RuntimeException(
    ...
    /** @see vrml.eai.field.EventInMFRotation#setValue */
    @Override
    public void setValue(float[][] newValues) {
        ...
        if (!hasStoredValue) {
            ...
            queuedElement=new EventInMFRotationWrapper(
                theNode,theFieldID,theQueue,newValues
            );
            theQueue.processEvent(queuedElement);
    }

    /** @see vrml.eai.field.EventInMFRotation#set1Value */
    @Override
    public void set1Value(int index, float[] aValue) {
        synchronized(theQueue.eventLock) {
            EventInMFRotationWrapper queuedElement=(EventInMFRotationWrapper)
    ...
    private void storeValue(float[][] newValue) {
        hasStoredValue=true;
        if (newValue!=null) {
            if (storedValue==null || storedValue.length!=(4*newValue.length))
                storedValue=new float[4*newValue.length];
            ArrayUtils.flatten4(newValue,newValue.length,storedValue);
        } else
            storedValue=null;
    }

    /** Modify the stored value at one index.
      * @param index The index to change
      * @param newValue The new value for that index */
    private void store1Value(int index, float[] newValue) {
        isSet1Value=true;
        System.arraycopy(newValue,0,storedValue,index*4,4);
    }
