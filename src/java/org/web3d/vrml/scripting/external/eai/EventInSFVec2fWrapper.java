...
/**
 *  EventInSFVec2fWrapper provides the functional implementation of
 *  EventInSFVec2f for the SimpleBrowser class.  The functionality of this class
 *  is specified in the EAI 2.0 specification.
 */

class EventInSFVec2fWrapper extends EventInSFVec2f
implements ExternalEvent, EventWrapper {
    ...
    /** Constructor for use in event queueing system.
      ...
      * @param id The underlying target field
      * @param aQueue The queue to send events to
      * @param aValue The value for this event (copied)
      */
    EventInSFVec2fWrapper(
        VRMLNodeType aNode, int id, ExternalEventQueue aQueue, float aValue[]
    ) {
    ...
    public void doEvent() {
        try {
            try {
                theNode.setValue(theFieldID,storedValue,2);
            } finally {
    ...
    /** @see vrml.eai.field.EventInSFVec2f#setValue */
    @Override
    public void setValue(float[] newValue) {
        ...
        if (!hasStoredValue) {
            ...
            queuedElement=new EventInSFVec2fWrapper(
    ...
    /** Store a value in this EventIn as a place holder for the buffering
      * system.
      * @param aValue The value to store (copied)
      */
    private void storeValue(float aValue[]) {
        hasStoredValue=true;
        storedValue[0]=aValue[0];
        storedValue[1]=aValue[1];
    }
