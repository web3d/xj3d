...
/**
 *  EventOutMFRotationWrapper provides the functional implementation of
 *  EventOutMFRotation for the SimpleBrowser class.  The functionality of 
 *  this class is specified in the EAI 2.0 specification.
 */

class EventOutMFRotationWrapper extends EventOutMFRotation 
implements ExternalOutputBuffer, EventWrapper {
    /** Indicates whether to load the value locally or retrieve from node */
    boolean isStored;

    /** The number of elements in the storeValue array (as opposed to the
      * array length). */
    int storedSize;
    ...
    public void getValue(float dest[]) {
        if (isStored)
            ...
            try {
                ...
                System.arraycopy(fieldData.floatArrayValues,0,dest,0,fieldData.numElements*4
                );
    ...
    /** @see vrml.eai.field.EventOutMFRotation#get1Value */
    @Override
    public void get1Value(int index, float[] dest) {
        if (isStored)
            System.arraycopy(storedValue,index*4,dest,0,4);
        else
            try {
                VRMLFieldData fieldData=theNode.getFieldValue(theFieldID);
                if (index<0 || index>= fieldData.numElements)
                    throw new ArrayIndexOutOfBoundsException();
                System.arraycopy(fieldData.floatArrayValues, index*4, dest, 0, 4
                );
            } catch (InvalidFieldException ife) {
    ...
    public void getValue(float[][] dest) {
        if (isStored) {
            ArrayUtils.raise4(storedValue,storedSize,dest);
        } else
            try {
                VRMLFieldData fieldData=theNode.getFieldValue(theFieldID);
                int arraySize=fieldData.numElements;
                ArrayUtils.raise4(fieldData.floatArrayValues,arraySize,dest);
            } catch (InvalidFieldException ife) {
    ...
    public float[][] getValue() {
        ...
        for (int counter=0; counter<result.length; counter++)
            result[counter]=new float[4];
        if (isStored) {
            ArrayUtils.raise4(storedValue,storedSize,result);
            return result;
        } else
            try {
                VRMLFieldData fieldData=theNode.getFieldValue(theFieldID);
                int arraySize=fieldData.numElements;
                ArrayUtils.raise4(fieldData.floatArrayValues,arraySize,result);
                return result;
            } catch (InvalidFieldException ife) {
                throw new RuntimeException("Error reading field value");
        }
    ...
    public void loadOutputValue() {
        ...
        storedSize=size();
        if (storedValue==null || storedValue.length!=(4*storedSize))
            storedValue=new float[4*storedSize];
