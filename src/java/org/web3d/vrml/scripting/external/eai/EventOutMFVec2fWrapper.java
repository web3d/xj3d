...
implements ExternalOutputBuffer, EventWrapper {
    ...
    public float[][] getValue() {
        ...
        if (isStored) {
            ...
            try {
                ...
                ArrayUtils.raise2(fieldData.floatArrayValues,fieldData.numElements,result
                );
                return result;
    ...
    public void getValue(float[][] dest) {
        if (isStored)
            ...
            try {
                VRMLFieldData fieldData=theNode.getFieldValue(theFieldID);
                ArrayUtils.raise2(fieldData.floatArrayValues, fieldData.numElements, dest
                );
            } catch (InvalidFieldException ife) {
                throw new RuntimeException("Error reading field value");
            }
    ...
    public void getValue(float[] dest) {
        if (isStored)
            ...
            try {
                VRMLFieldData fieldData=theNode.getFieldValue(theFieldID);
                System.arraycopy(fieldData.floatArrayValues, 0, dest, 0, 
    ...
    public void get1Value(int index, float[] dest) {
        if (isStored)
            ...
            try {
                ...
                System.arraycopy(fieldData.floatArrayValues, index+index, dest, 0, 2
                );
            } catch (InvalidFieldException ife) {
                throw new RuntimeException("Error reading field value");
            }
