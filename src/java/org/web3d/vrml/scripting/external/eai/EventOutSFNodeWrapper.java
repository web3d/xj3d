...
/**
 *  EventOutSFNodeWrapper provides the functional implementation of
 *  EventOutSFNode for the SimpleBrowser class.  The functionality of this class
 *  is specified in the EAI 2.0 specification.
 */

class EventOutSFNodeWrapper extends EventOutSFNode 
implements ExternalOutputBuffer, EventWrapper {
    ...
    public Node getValue() {
        if (isStored)
            ...
            try {
                return theNodeFactory.getEAINode((VRMLNodeType)(
                    theNode.getFieldValue(theFieldID).nodeValue)
                );
