...
/**
 *  EventOutSFVec3fWrapper provides the functional implementation of
 *  EventOutSFVec3f for the SimpleBrowser class.  The functionality of this 
 *  class is specified in the EAI 2.0 specification.
 */

class EventOutSFVec3fWrapper extends EventOutSFVec3f 
implements ExternalOutputBuffer, EventWrapper {
    ...
    /** @see vrml.eai.field.EventOutSFVec3f#getValue */
        @Override
    public void getValue(float dest[]) {
        if (isStored)
            System.arraycopy(storedValue,0,dest,0,3);
        else 
            try {
            	VRMLFieldData value=theNode.getFieldValue(theFieldID);
            	if (value.floatArrayValues==null)
            		System.arraycopy(DEFAULT_FIELD_VALUE,0,dest,0,3);
            	else
