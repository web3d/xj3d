...
import vrml.eai.field.EventInSFRotation;
 ...
 */
public class EventInSFRotationWrapper extends EventInSFRotation
     ...
     */
    public EventInSFRotationWrapper(
        int fieldID, 
        FieldAndNodeRequestProcessor requestProcessor,
        NetworkEventQueue eventQueue
    ) {
    ...
    /** * @see vrml.eai.field.EventInSFRotation#setValue(float[])  */
    @Override
    public void setValue(float[] value) {
        EventInSFRotationWrapper queuedElement;
        if (!hasStoredValue)
            queuedElement=this;
        else
            queuedElement=new EventInSFRotationWrapper(fieldID,requestProcessor,eventQueue);
        if (queuedElement.storedValue==null || queuedElement.storedValue.length!=value.length)
            queuedElement.storedValue=new float[4];
        System.arraycopy(value,0,queuedElement.storedValue,0,4);
    ...
    public void writeFieldValue(DataOutputStream output) throws IOException {
        ...
        output.writeFloat(storedValue[3]);
    }
}
