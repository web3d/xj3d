...
public class EventInSFVec2fWrapper extends EventInSFVec2f 
    ...
    /** * @see vrml.eai.field.EventInSFVec2f#setValue(float[])  */
    @Override
    public void setValue(float[] value) {
        EventInSFVec2fWrapper queuedElement;
        if (!hasStoredValue)
            queuedElement=this;
        else
            queuedElement=new EventInSFVec2fWrapper(fieldID,requestProcessor,eventQueue);
        if (queuedElement.storedValue==null || queuedElement.storedValue.length!=value.length)
            queuedElement.storedValue=new float[2];
        System.arraycopy(value,0,queuedElement.storedValue,0,2);
