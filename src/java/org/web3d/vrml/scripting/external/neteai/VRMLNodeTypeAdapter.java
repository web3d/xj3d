...
public class VRMLNodeTypeAdapter implements VRMLNodeListener {
    ...
    IntHashMap<Integer> globalFieldIDToLocalFieldIDTable;

    /** Mapping from the field index on the node to the network field ID */
    IntHashMap<Integer> localFieldIDToGlobalFieldIDTable;

    /** If a field has listeners, then it will have
     *  a non-null entry here.
     */
    IntHashMap<VRMLNodeTypeAdapter> listenerTable;
    ...
    public void activateFieldListener(int globalFieldID) {
        Integer field=globalFieldIDToLocalFieldIDTable.get(globalFieldID);
        if (field==null)
    ...
    int getLocalFieldID(int networkFieldID) {
        Integer i=globalFieldIDToLocalFieldIDTable.get(networkFieldID);
        if (i==null)
            throw new InvalidFieldException();
        else
            return i;
    ...
    public int registerField(int xj3dFieldID) {
        int fieldID;
        Integer i=localFieldIDToGlobalFieldIDTable.get(xj3dFieldID);
        if (i==null) {
            fieldID=globalIDGenerator.generateID();
            globalFieldIDToLocalFieldIDTable.put(fieldID, xj3dFieldID);
            localFieldIDToGlobalFieldIDTable.put(xj3dFieldID, fieldID);
        } else
            fieldID=i;
        return fieldID;
    }
    ...
    public void releaseField(int globalFieldID) {
        Integer localField=globalFieldIDToLocalFieldIDTable.get(globalFieldID);
        if (localField==null)
            throw new InvalidFieldException();
        int localFieldID=localField;
    ...
    public void fieldChanged(int index) {
        if (listenerTable.get(index)==null)
            return;
        Integer field=localFieldIDToGlobalFieldIDTable.get(index);
        if (field==null) {
            // No listeners
        }
        else {
            int globalFieldID=field;
