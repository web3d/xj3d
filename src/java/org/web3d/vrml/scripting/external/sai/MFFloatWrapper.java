...
/**
 * Implementation of the MFFloat wrapper.
 *
 */
class MFFloatWrapper extends BaseFieldWrapper
    implements MFFloat, ExternalEvent, ExternalOutputBuffer {
    ...
    /**
     * Constructor to use when a value is not preloaded
     ...
     */
    MFFloatWrapper(VRMLNodeType node, int field, ExternalEventQueue aQueue,
    ...
    public void append(float value) {
        ...
        synchronized(theEventQueue.eventLock) {
            MFFloatWrapper queuedElement=
                (MFFloatWrapper) theEventQueue.getLast(this);
            ...
            if (queuedElement==null || !queuedElement.isSetOneValue) {
                ...
                if (!storedInput && !storedOutput) {
                    ...
                    queuedElement=new MFFloatWrapper(
    ...
    /**
     * @see org.web3d.x3d.sai.MFFloat#clear()
     */
    @Override
    public void clear() {
    ...
    public float get1Value(int index)
        ...
        if (storedOutput)
            ...
            if (index<0 || index>=value.numElements)
                ...
                return value.floatArrayValues[index];
        }
    }
    ...
    public void getValue(float[] values) {
        if (storedOutput)
            ...
            if (data.numElements!=0)
                System.arraycopy(data.floatArrayValues,0,values,0,data.numElements);
        }
    }
    ...
    /**
     * @see org.web3d.x3d.sai.MFFloat#insertValue(int, float)
     */
    @Override
    ...
    private void loadInputValue() {
        ...
        if (storedInputValue == null || storedInputValue.length!=value.numElements)
            storedInputValue=new float[value.numElements];
        if (value.numElements!=0)
            System.arraycopy(value.floatArrayValues,0,storedInputValue,0,value.numElements);
        storedInput=true;
    ...
    public void loadOutputValue() {
        ...
        if (storedOutputValue == null || storedOutputValue.length!=value.numElements)
            storedOutputValue=new float[value.numElements];
        if (value.numElements!=0)
            System.arraycopy(value.floatArrayValues,0,storedOutputValue,0,value.numElements);
        storedOutput=true;
    }

    /**
     * @see org.web3d.x3d.sai.MFFloat#removeValue(int)
     */
    @Override
        ...
        synchronized (theEventQueue.eventLock) {
            ...
            if (queuedElement == null || !queuedElement.isSetOneValue) {
                ...
                if (!storedInput && !storedOutput) {
                    ...
                    queuedElement = new MFFloatWrapper(theNode, fieldIndex,
                            theEventQueue, theEventAdapterFactory, true);
                    queuedElement.isSetOneValue = true;
                }
                newEvent=true;
            }
     ...
     *
     * @param size
     *            The number of items to copy from the array
     * @param value
    ...
    public void setValue(int size, float[] value) {
        checkWriteAccess();
        MFFloatWrapper queuedElement=this;
        // Input and output buffers do not mix and further don't
        // overwrite set1Value calls
        if (isSetOneValue || storedInput || storedOutput) {
            queuedElement=new MFFloatWrapper(theNode, fieldIndex,
                                             theEventQueue, theEventAdapterFactory);
        }
        queuedElement.storedInput=true;
        if (queuedElement.storedInputValue==null || queuedElement.storedInputValue.length!=size)
            queuedElement.storedInputValue=new float[size];
