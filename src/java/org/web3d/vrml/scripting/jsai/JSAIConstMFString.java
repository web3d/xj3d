 ...
 */
class JSAIConstMFString extends ConstMFString
     ...
     */
    JSAIConstMFString(VRMLNodeType n, int index) {
        node = n;
        fieldIndex = index;
        valueChanged = true;
        updateLocalData();
    ...
    @Override
    public Object clone() {
        return new JSAIConstMFString(node, fieldIndex);
    }
    ...
    private void updateLocalData() {
        ...
        try {
            ...
            if (fd.numElements == 0) {
                ...
                setValue(fd.numElements, fd.stringArrayValues);
            valueChanged = false;
     ...
     */
    private void setValue(int size, String[] values) {
        if(size > data.length)
